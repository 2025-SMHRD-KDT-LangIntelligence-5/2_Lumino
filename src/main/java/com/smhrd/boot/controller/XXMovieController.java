...
public class XXMovieController {
...
//    }
    ...
    @PostMapping("/movies")
    public RedirectView addMovie(@ModelAttribute XXMovie movie){
        // @RequestParam ; 파라미터를 하나하나 받는 방법
        // @ModelAttribute : 모델(객체) 형태로 묶어서 받는 방법(Movie 객체 형태) -> 어노테이션 생략가능
        XXMovie result = service.addMovie(movie);

        return new RedirectView("/"); // 리다이렉팅 방식으로 "/" 경로로 다시 요청하게 해서 xxmovies.html 볼 수 있도록 함
    }
    ...
    public XXMovie getMovie(@PathVariable Long movieId){
        XXMovie movie = service.getMovie(movieId);
        return movie;
    }
}
